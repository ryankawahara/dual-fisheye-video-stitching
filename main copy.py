...
def convert_frame_to_video(file_path, fps, pathOut=dir_path+ '/output/'):
    frame_array = []
    ...
    out = cv2.VideoWriter(pathOut+"000.mp4",cv2.VideoWriter_fourcc(*'MPEG'), fps, size)
    print(frame_array)
    for i in range(24):
...
def video_convert(input, output):
    ...
    #         mask, minloc_old = optimal_seamline.imgLabeling(
    #             warped1[:, W_remap // 2 - W_lbl:W_remap // 2],
    #             warped2[:, W_remqap // 2 - W_lbl:W_remap // 2],
    #             warped1[:, W - W_remap // 2:W - W_remap // 2 + W_lbl],
    #             warped2[:, W - W_remap // 2:W - W_remap // 2 + W_lbl],
